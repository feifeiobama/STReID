...
class Market1501(Dataset):
    ...
    def download(self):
        ...
        def register(subdir, pattern=re.compile(r'([-\d]+)_c(\d)s(\d)_([\d]+)')):
            fnames = []  ######### Added. Names of images in new dir.
            fpaths = sorted(glob(osp.join(exdir, subdir, '*.jpg')))
            pids = set()
            
            timestamps = np.zeros((6, 6))
            for fpath in fpaths:
                fname = osp.basename(fpath)
                pid, cam, seq, timestamp = map(int, pattern.search(fname).groups())
                if pid == -1: continue  # junk images are just ignored
                assert 0 <= pid <= 1501  # pid == 0 means background
                assert 1 <= cam <= 6
                cam -= 1
                seq -= 1
                timestamps[cam, seq] = max(timestamps[cam, seq], timestamp)
            ...
            for fpath in fpaths:
                ...
                seq -= 1
                pids.add(pid)
                timestamp = int(timestamp + timestamps[cam, seq])
