...
class Evaluator(object):
    ...
    def evaluate(self, data_loader, query, gallery, st_model=None, metric=None):
        features, _ = extract_features(self.model, data_loader, print_freq=self.print_freq)
        # print('before re_ranking:')
        distmat = pairwise_distance(features, query, gallery, metric=metric)
        if st_model is not None:
            evaluate_all(distmat, query=query, gallery=gallery)
            distmat = st_model.apply(distmat, query, gallery)
        return evaluate_all(distmat, query=query, gallery=gallery)
        # print('applying re_ranking:')
        # distmat_qq = pairwise_distance(features, query, query, metric)
        # distmat_gg = pairwise_distance(features, gallery, gallery, metric)
        # distmat = re_ranking2(distmat.numpy(), distmat_qq.numpy(), distmat_gg.numpy())
        # return evaluate_all(distmat, query=query, gallery=gallery)
