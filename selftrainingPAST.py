...
def get_data(name, data_dir, height, width, batch_size,
    ...
    processed_data = Preprocessor(train_set, root=dataset.images_dir,
    ...
    return dataset, num_classes, extfeat_loader, test_loader, processed_data
...
def main(args):
    ...
    # get target data
    tgt_dataset, num_classes, tgt_extfeat_loader, test_loader, processed_data = \
    ...
    for iter_n in range(args.iteration):
        ...
        train_dataset = Preprocessor(new_dataset, root=tgt_dataset.images_dir, transform=train_transformer)
        ...
        # if iter_n % 2 == 0:
        #     # criterion[2] = RankingTripletLoss(args.margin, nums, isAvg=True).cuda()
        #     #
        #     # rerank_index = np.argsort(rerank_dist, axis=1)
        #     ranking = np.zeros(rerank_dist.shape)
        #     # for i in range(rerank_dist.shape[0]):
        #     #     for j in range(rerank_dist.shape[1]):
        #     #         ranking[i][rerank_index[i, j]] = j
        #     past_trainer = PastTrainer(model, new_dataset_index, ranking, train_loader, criterion, optimizer)
        #
        #     for epoch in range(args.epochs):
        #         past_trainer.train(epoch)
        #
        ...
        #     ], lr=args.lr)
        #
        #     classification_trainer = ClassificationTrainer(model, train_loader, criterion, classOptimizer)
        #
        #     for epoch in range(args.epochs):
        #         # trainer.train(epoch, remRate=0.2+(0.6/args.iteration)*(1+iter_n)) # to at most 80%
        #         classification_trainer.train(epoch)
        #
        #     rank_score = evaluator.evaluate(test_loader, tgt_dataset.query, tgt_dataset.gallery)
        ...
        criterion[2] = RankingTripletLoss(args.margin).cuda()
        ...
        past_trainer = PastTrainer(model, index_map, rerank_index, ranking, processed_data, nums, train_loader, criterion, classOptimizer)
