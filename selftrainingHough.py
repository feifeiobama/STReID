...
from reid.hough import Hough
from reid.loss import HoughTripletLoss
from reid.trainers import HoughTrainer
...
def main(args):
    ...
    # Criterion
    criterion = [
        HoughTripletLoss(args.margin, args.num_instances, isAvg=False, use_semi=False).cuda(),
        HoughTripletLoss(args.margin, args.num_instances, isAvg=False, use_semi=False).cuda()
    ]
    ...
    for iter_n in range(args.iteration):
        ...
        for (fname, realPID, cam, timestamp), label in zip(tgt_dataset.trainval, labels):
            if label == -1:
                unknown_dataset.append((fname, int(unknownLab[unCounter]), cam, timestamp))  # unknown data
            ...
            for index2 in record_labels[label]:
                hough.update(cam, tgt_dataset.trainval[index2][2], timestamp, tgt_dataset.trainval[index2][3])
        ...
        for (fname, realPID, cam, timestamp), label in zip(tgt_dataset.trainval, labels):
            ...
            for index2 in realIDs[realPID]:
                ...
                if hough.on_peak(cam, tgt_dataset.trainval[index2][2], timestamp, tgt_dataset.trainval[index2][3]):
                    TP_gt += 1
        ...
        print('gt label: T = %d, TP = %d, recall = %f' % (T_gt, TP_gt, TP_pseu / T_gt))
        print('pseudo label: T = %d, TP = %d, recall = %f' % (T_pseu, TP_pseu, TP_pseu / T_pseu))
        ...
        trainer1 = HoughTrainer(
            model, hough, train_loader, criterion, optimizer
        )
        trainer2 = HoughTrainer(
            model, hough, unLoader, criterion, optimizer
        )
...
if __name__ == '__main__':
    ...
    # hough
    parser.add_argument('--short_cut', action='store_true', help='disable hough filtering')
